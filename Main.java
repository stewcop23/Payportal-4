  ...
  //  private static Scanner scan = new Scanner(System.in);
    ...
    public static ArrayList<User> getUsers(){
        ...
        if (mode.equals("Admin")) {
            ...
            User.addtoArray(new CardHolder(uname, pass, extra.split(",")[0], extra.split(",")[1], extra.split(",")[2], new Scanner(System.in), false));
            for(int i = 0;i<index;i++){
                ...
                if(type.equals("V")){
                    ((CardHolder)User.getUserArray().get(User.getUserIndex(uname))).addCard(new Visa(number, expmoth, expyear, cvv, ballance));
                }
                else if(type.equals("D")){
                    ((CardHolder)User.getUserArray().get(User.getUserIndex(uname))).addCard(new Debit(number, expmoth, expyear, cvv, ballance));
                }
